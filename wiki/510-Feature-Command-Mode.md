...
The speed changes in real time as you press the paddles. Squeeze both paddles together, or press the command button, to exit and save the new speed — this mode reads the paddles directly for speed control, so keying `X` has no effect here.
...
PTT and TX key line behave normally in tune mode. Exit by squeezing both paddles together, or by pressing the command button — as in Speed Adjust Mode, the paddles are read directly here, so keying `X` doesn't exit.
...
| CW after P | Memory |
|-----------|--------|
| `.----` (standard CW numeral 1) | Memory 1 |
| `..---` (standard CW numeral 2) | Memory 2 |
| `...--` (standard CW numeral 3) | Memory 3 |
...
`command_mode_active_led` exists as a pin setting in `keyer_2_pin_settings.h`, but it isn't wired to any logic yet — there's no code anywhere that lights it. Setting it to a pin number currently has no effect.
